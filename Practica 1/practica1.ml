...
let cadena = [Terminal "a";Terminal "a";Terminal "a";Terminal "b";Terminal "a";Terminal "b"];;
let cadena2 = [Terminal "b";Terminal "b"];;
...
let finales estados_afd estados_finales =
  let rec aux = function
    ...
    | (h::t,estados_finales) ->
      ...
      in  union (recorrido(h,estados_finales))  (aux(t,estados_finales))
...
let arcos c (Af (_, alfabeto, _, _, _) as a) =
  let rec aux = function
    ...
    | (h::t,alfabeto,a) ->
      ...
      in union (recorrido (h,alfabeto,a,h::t)) (aux (t,alfabeto,a,h::t))
  in aux(list_of_conjunto c,list_of_conjunto alfabeto,a)
;;
...
let afd_of_afn (Af (_, alfabeto, inicial, _, _) as a) =
  ...
  let rec aux = function
  (Af (_, alfabeto, _, _, estados_finales) as a,([],conjunto_tratado),_) -> Af((estados_afd (list_of_conjunto conjunto_tratado)), alfabeto, Estado "0", (arcos conjunto_tratado a), finales conjunto_tratado estados_finales)
